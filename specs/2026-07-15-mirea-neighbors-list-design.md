...
- `deploy/publish-dashboard.sh` — добавить копирование `docs/mirea-list.html` в gh-pages
  worktree (по образцу строки для `mirea-scores.html`), иначе страница не попадёт на сервер.
